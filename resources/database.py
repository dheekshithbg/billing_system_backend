...
def get_bills_by_user(user_id, status=None):
    """Retrieve all bills for a specific user, optionally filtered by status."""
    conn = create_connection()
    bills = []
    if conn:
        try:
            cursor = conn.cursor()
            if status:
                cursor.execute("SELECT * FROM bills WHERE user_id = ? AND status = ? ORDER BY due_date ASC;", (user_id, status))
            else:
